...
TOP_K_FACTOR_SEMANTIC = 5 # 语义通道候选池放大倍数
TOP_K_FACTOR_KEYWORD = 3  # 关键词通道候选池放大倍数
...
class RAGEngine:
    ...
    def search(
        ...
        for c in candidates:
            ...
            doc_texts.append(text[:2000])  # 限制文本长度
    ...
    def _rerank(
        ...
        try:
            ...
            # BGE-reranker 返回: {"results": [{"index": 0, "relevance_score": 0.98}, ...]}
            if "results" in data:
                ranked = sorted(
                    data["results"],
            ...
            logger.warning(f"[RAG] Reranker 失败, 降级为余弦排序: {e}")
            pass

        # 降级: 用原始分数排序
        ranked = sorted(enumerate(docs), key=lambda x: -len(x[1]))
        return [(i, 0.0) for i, _ in ranked[:top_k]]
    ...
    def health(self) -> dict:
        ...
        status = {}
        try:
            self._http.get(f"{self._bge_url}/health", timeout=5)
            status["bge"] = "ok"
        except Exception:
            status["bge"] = "error"
